...
import { Box, Button, Flex, InjectedModalProps, LinkExternal, Message, Skeleton, Text } from '@gravyswap/uikit'
...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: gravyBalance, fetchStatus: gravyFetchStatus } = useTokenBalance(tokens.gravy.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('GRAVY Balance')}</Text>
        {gravyFetchStatus !== FetchStatus.SUCCESS ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(gravyBalance, 18, 3)}</Text>
        )}
      </Flex>
