...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@gravyswap/uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://gravyswap.com - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/gravyswap.com)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://gravyswap.com'}
          color={text === 'https://gravyswap.com' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
