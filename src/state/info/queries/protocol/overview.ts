...
const formatGravyFactoryResponse = (rawGravyFactory?: GravyFactory) => {
  if (rawGravyFactory) {
    return {
      totalTransactions: parseFloat(rawGravyFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawGravyFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawGravyFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatGravyFactoryResponse(data?.gravyFactories?.[0])
      const overviewData24 = formatGravyFactoryResponse(data24?.gravyFactories?.[0])
      const overviewData48 = formatGravyFactoryResponse(data48?.gravyFactories?.[0])
