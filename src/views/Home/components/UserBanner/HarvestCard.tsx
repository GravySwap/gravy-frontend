...
import { AutoRenewIcon, Button, Card, CardBody, Flex, Skeleton, Text, ArrowForwardIcon } from '@gravyswap/uikit'
...
const HarvestCard = () => {
  ...
  const earningsBusd = new BigNumber(farmEarningsSum).multipliedBy(gravyPriceBusd)
  ...
  const hasGravyPoolToCollect = numTotalToCollect - numFarmsToCollect > 0

  const earningsText = getEarningsText(numFarmsToCollect, hasGravyPoolToCollect, earningsBusd, t)
  ...
  const harvestAllFarms = useCallback(async () => {
    ...
    for (const farmWithBalance of farmsWithStakedBalance) {
      try {
        ...
        toastSuccess(
          `${t('Harvested')}!`,
          t('Your %symbol% earnings have been sent to your wallet!', { symbol: 'GRAVY' }),
        )
      } catch (error) {
