...
import { fetchGravyVaultFees, fetchPoolsPublicDataAsync } from 'state/pools'
import { simpleRpcProvider } from 'utils/providers'
import { useGravyVault, usePools } from 'state/pools/hooks'
...
const useGetTopPoolsByApr = (isIntersecting: boolean) => {
  ...
  const pools = useMemo(() => {
    ...
    const gravyPool = activePools.find((pool) => pool.sousId === 0)
    const gravyAutoVault = { ...gravyPool, isAutoVault: true }
    const gravyAutoVaultWithApr = { ...gravyAutoVault, apr: getAprData(gravyAutoVault, performanceFeeAsDecimal).apr }
    return [gravyAutoVaultWithApr, ...poolsWithoutAutoVault]
  ...
  useEffect(() => {
    const fetchPoolsPublicData = async () => {
      ...
      try {
        await dispatch(fetchGravyVaultFees())
  ...
  }, [setTopPools, pools, fetchStatus, gravyPriceBusd, topPools, performanceFeeAsDecimal])
