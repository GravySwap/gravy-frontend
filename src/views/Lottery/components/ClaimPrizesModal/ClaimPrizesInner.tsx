...
import { Flex, Button, Text, AutoRenewIcon, PresentWonIcon } from '@gravyswap/uikit'
...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const gravyReward = activeClaimData.gravyTotal
  const dollarReward = gravyReward.times(gravyPriceBusd)
  const rewardAsBalance = getBalanceAmount(gravyReward).toNumber()
      ...
      if (receipt.status) {
        toastSuccess(
          ...
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('Your GRAVY prizes for round %lotteryId% have been sent to your wallet', { lotteryId })}
