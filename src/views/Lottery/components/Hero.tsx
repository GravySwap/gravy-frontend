...
import { Box, Flex, Heading, Skeleton } from '@gravyswap/uikit'
...
const Hero = () => {
  ...
  const {
    currentRound: { amountCollectedInGravy, status },
    isTransitioning,
  } = useLottery()

  const gravyPriceBusd = usePriceGravyBusd()
  const prizeInBusd = amountCollectedIngravy.times(gravyPriceBusd)
