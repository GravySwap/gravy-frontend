...
const useAllGravyBunnyNfts = (collectionAddress: string) => {
  ...
  const isPBCollection = collectionAddress === gravynniesAddress
  ...
  useEffect(() => {
    const fetchGravyBunnies = async () => {
      ...
      const { data } = await getNftsFromCollectionApi(gravynniesAddress)
      ...
      const allBunnies: NftToken[] = bunnyIds.map((bunnyId) => {
        return {
          ...
          collectionAddress: gravynniesAddress,
